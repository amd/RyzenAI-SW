...
- [3 ONNX Benchmark](#3-onnx-benchmark)
  ...
  - [3.3 Example Usage](#33-example-usage)
    - [3.3.1 A GUI for performance benchmarking](#331-a-gui-for-performance-benchmarking)
    ...
    - [3.3.5 Best Throughput: performance with eight instances of 1x4 core with NPU](#335-best-throughput-performance-with-eight-instances-of-1x4-core-with-npu)
    - [3.3.6 Best Latency: performance with one instance of 4x4 core with NPU](#336-best-latency-performance-with-one-instance-of-4x4-core-with-npu)
...
:pushpin: NOTE:  This tool is currently supported only on the Ryzen processor "STRIX". 

## 1.1 <a name='Notesforthisrelease'></a>Notes for this release
Windows release for STRIX device.
...
The following guideline allows for the creation of a Conda environment with  CPU and VitisAI Execution Providers only.

Follow the instructions from the official [RyzenAI 1.2 Installation](https://ryzenai.docs.amd.com/en/develop/inst.html).
...
```
Finally, set environment variables, using the Ryzen AI installation path `RYZEN_AI_INSTALLATION_PATH`:
```
set XCLBINHOME=%RYZEN_AI_INSTALLATION_PATH%\voe-4.0-win_amd64\xclbins\strix
...
```
  ...
  --core {STX_1x4,STX_4x4}
                        Which core to use on STRIX silicon. Possible values are 1x4 and 4x4. Default=STX_1x4
  --device {CPU,VitisAIEP,iGPU,dGPU}, -d {CPU,VitisAIEP}
...
###  3.3.1 <a name='AGUIforperformancebenchmarking'></a>A GUI for performance benchmarking
This straightforward graphical user interface is designed to help users compose the command line and launch the program with all parameters preset. All option defaults are preloaded.
```
python gui_pb.py
```
...
```
python performance_benchmark.py -m .\models\resnet50\resnet50_fp32.onnx -n 100 -d VitisAIEP --calib .\<images folder> --num_calib 10
```
...
```
python performance_benchmark.py -m .\models\resnet50\resnet50_fp32_qdq.onnx -n 100 -d VitisAIEP
```


###  3.3.5 <a name='PerformancewithfourinstanceswithNPUandforcerecompiling'></a>Best Throughput: performance with eight instances of 1x4 core with NPU 
The default core 1x4 is implemented in AMD_AIE2P_Nx4_Overlay.xclbin and is optimized for flexibility and throughput.
The best throughput is achieved when eight instances are used. It is recommended to use more than eight threads for optimal performance.
...
```
python performance_benchmark.py -m .\models\resnet50\resnet50_fp32_qdq.onnx -n 100 -d VitisAIEP -i 8 -t 12 -r 1
```

###  3.3.6 <a name='PerformancewithwithNPUoneinstanceof4x4core'></a>Best Latency: performance with one instance of 4x4 core with NPU
The core 4x4 is implemented in AMD_AIE2P_4x4_Overlay.xclbin and is optimized for latency.
In this example, the 4x4 core is used ```--core STX_4x4```. If you have been following the sequence of experiments, previously the model was compiled for the default STX_1x4 core configuration. Therefore, the cache needs to be cleared using ```-r 1```, and the model needs to be recompiled for the new core configuration.
```
python performance_benchmark.py -m .\models\resnet50\resnet50_fp32_qdq.onnx -n 100 -d VitisAIEP -i 1 -t 1 -r 1 --core STX_4x4
```
...
```
python performance_benchmark.py --json .\test\resnet50_high_throughput.json
python performance_benchmark.py --json .\test\resnet50_low_latency.json
```
...
```
python performance_benchmark.py -m ".\models\resnet50\resnet50_fp32.onnx" -d VitisAIEP --calib ".\Imagenet_small" --num_calib 10
```

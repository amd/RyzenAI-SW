...
- [Setup](#setup)
  - [Install Procedure](#install-procedure)
    - [Verify the EP available in the environment](#verify-the-ep-available-in-the-environment)
- [ONNX Benchmark](#onnx-benchmark)
  - [Parameters](#parameters)
  - [Measurement Report](#measurement-report)
  - [Example Usage](#example-usage)
    - [1. Performance with a single instance using a CPU](#1-performance-with-a-single-instance-using-a-cpu)
    - [2. Performance with single instance with NPU](#2-performance-with-single-instance-with-npu)
    - [3. Performance with four instances with NPU and force recompiling](#3-performance-with-four-instances-with-npu-and-force-recompiling)
    - [4. Performance with with NPU, one instance of 4x4 core](#4-performance-with-with-npu-one-instance-of-4x4-core)
    - [5. Performance with single instance with CPU at 30fps fixed rate](#5-performance-with-single-instance-with-cpu-at-30fps-fixed-rate)
    - [6. File of parameters](#6-file-of-parameters)
- [Auto quantization feature (beta)](#auto-quantization-feature-beta)
...
| Complete the benchmark setup | Once activated the conda environment with ```conda activate ryzenai-<release>```, run the batch script ```setup.bat``` : this will copy locally all xclbin files and add additional packages |


#### Verify the EP available in the environment
...
```
usage: performance_benchmark.py [-h] 

[--batchsize BATCHSIZE] 
[--calib CALIB] 
[--config CONFIG]
[--core CORE]
[--device {CPU,iGPU,dGPU,ZenDNN,VitisAIEP}] 
[--infinite INFINITE] 
[--instance-count INSTANCE_COUNT]                           
[--intra-op-num-threads INTRA_OP_NUM_THREADS] 
[--json JSON] 
[--log_csv LOG_CSV] 
[--log_json LOG_JSON] 
...
```
  ...
  --calib CALIB         path to Imagenet database, used for quantization with calibration. Default= .\Imagenet
  --config CONFIG, -c CONFIG
                        path to config json file. Default=vaip_config.json
  --core CORE           Which core to use. Possible values are 1x4 and 4x4. Default=1x4
  --device {CPU,iGPU,dGPU,ZenDNN,VitisAIEP}, -d {CPU,iGPU,dGPU,ZenDNN,VitisAIEP}
                        Execution Provider selection. Default=CPU
  --infinite INFINITE   if 1: Executing an infinite loop, when combined with a time limit, enables the test to run for a specified duration. Default=1
  --instance-count INSTANCE_COUNT, -i INSTANCE_COUNT
                        This parameter governs the parallelism of job execution. When the Vitis AI EP is selected, this parameter controls the number of DPU runners. The workload is always equally divided per each instance count. Default=1
  --intra-op-num-threads INTRA_OP_NUM_THREADS
                        In general this parameter controls the total number of INTRA threads to use to run the model.
                        INTRA = parallelize computation inside each operator.
                        Specific for VitisAI EP: number of CPU threads enabled when an operator is resolved by the CPU. Affects the performances but also the CPU power consumption. 
                        For best performance: set intra_op_num_threads to 0. Default: (not specified or 0) INTRA Threads Total = Number of physical CPU Cores.
                        For best power efficiency: set intra_op_num_threads to smallest number (>0) that sustains the close to optimal performance (likely 1 for most cases with 99% of model running on DPU)
  ...
  --no-inference NO_INFERENCE
                        When set to 1 the benchmark runs without inference for power measurements baseline. Default=0
...
Saving as a JSON file is facilitated by using the option ```--log-json``` or ```-j```, while saving as a CSV file is enabled with the option ```--log-csv``` or ```-k```.

### Example Usage

#### 1. Performance with a single instance using a CPU
...
#### 2. Performance with single instance with NPU
This experiment involves repeatedly performing inferences on a batch of 100 images ```-n 100```, sending one image at a time to the NPU ```-d VitisAIEP```: most operators are processed by the NPU. When the NPU is used it is necessary a configuration file ```--config .\models\resnet50\vaip_config.json```. The compiler will load the quantized model ```-m .\models\resnet50\resnet50_fp32_qdq.onnx```, and store the compiled model in a cache folder. Finally, the images are sent to the compiled model to be processed. The provided information includes the ratio of operators assigned to the CPU and those assigned to the NPU.
```
python performance_benchmark.py -m .\models\resnet50\resnet50_fp32_qdq.onnx -n 100 -d VitisAIEP --config .\models\resnet50\vaip_config.json
```


#### 3. Performance with four instances with NPU and force recompiling 
...
```
python performance_benchmark.py -m .\models\resnet50\resnet50_fp32_qdq.onnx -n 100 -d VitisAIEP --config .\models\resnet50\vaip_config.json -i 4 -t 4 -r 1
```


#### 4. Performance with with NPU, one instance of 4x4 core
...
```
python performance_benchmark.py -m .\models\resnet50\resnet50_fp32_qdq.onnx -n 100 -d VitisAIEP --config .\models\resnet50\vaip_config.json -i 1 -t 1 -r 1 --core 4x4
```

#### 5. Performance with single instance with CPU at 30fps fixed rate
In certain scenarios, it might be necessary to fine-tune the minimum latency, and this can be accomplished through the ```--min-interval``` option. For instance, adding a delay of 0.033 seconds is employed to simulate live video at a consistent 30 frames per second. Thus, we set the delay to 1/30 (0.033 s). In this context, the option is utilized alongside multithreading to ensure the CPU can maintain the desired framerate, even if the latency exceeds 33 milliseconds.
```
python performance_benchmark.py -m .\models\resnet50\resnet50_fp32.onnx -n 100 -d CPU -t 2 --min-interval 0.033 -r 1
```

#### 6. File of parameters
...
## Auto quantization feature (beta)
If the model benchmarked with VitisAI EP is not quantized, the benchmarking process will automatically apply quantization using the NHWC order. A notification in magenta will be displayed during the quantization process. It's important to note that this feature is still in its preliminary stage.
Prior to quantization, the user is required to download the ImageNet database and specify the path to its location. Once quantization is completed, a new model named NHWC_qdq_U8S8.onnx will be saved in the current directory.
```
python performance_benchmark.py -m "<path to the onnx model>\mobilenet_v2.onnx" -d VitisAIEP --calib "<path to Imagenet database>\Imagenet\val" --config "<path to vaip_config.json>\vaip_config.json -r 1
```

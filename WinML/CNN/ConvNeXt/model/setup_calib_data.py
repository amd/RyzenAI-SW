#!/usr/bin/env python3
"""
Setup calibration data for ConvNeXt INT8 quantization
...
def download_coco128(num_images):
    """Download COCO128 subset from Ultralytics GitHub release. No authentication required."""
...
def main():
    parser = argparse.ArgumentParser(
        description="Download calibration data for ConvNeXt quantization",
...
Examples:
  ...
  python setup_calib_data.py --source local --val-dir D:/datasets/ILSVRC2012/val
    ...
    parser.add_argument('--source', type=str, default='coco128', choices=['coco128', 'local'],
    ...
    print("=" * 70)
    print("ConvNeXt Calibration Data Setup")
    print("=" * 70)
    ...
    if CALIB_DIR.exists():
        ...
        if existing > 0 and not args.force:
            ...
            print("\nNext step:")
            print("  python quantize_convnext.py")
            return 0
        elif args.force:
    ...
    if not success:
        ...
        print(f"  # Then run: python quantize_convnext.py")
        return 1
    ...
    print(f"\nNext step:")
    print(f"  python quantize_convnext.py")
    print(f"{'=' * 70}")

...
When users fine-tune these models, only the weights change and no new operator shapes are introduced. In that case, follow the steps from [Preparing OGA Models](/llms/oga_model_prepare) to prepare the model, which will run on the Ryzen AI runtime using the precompiled operators.
...
1. Ensure the model is quantized following the [quantization recipe](/llms/oga_model_prepare#quantization).
...
Follow the [official guide](/llms/hybrid_oga) to copy `model_benchmark.exe` and required DLL dependencies to the current working directory. Then run:

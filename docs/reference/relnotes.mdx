...
- Breaking Changes
  ...
  - For running INT8 models on STX/KRK or newer devices, the `xclbin` provider option is no longer supported and should no longer be used. See [Using INT8 Models](/tools/modelrun#int8-models) for full details.
  - For running INT8 models on PHX/HPT devices, the `target` option should be set to `X1`. The NPU binary should still be specified using the `xclbin` provider option. See [Using INT8 Models](/tools/modelrun#int8-models) for full details.
  - For BF16 models, the default configurations file requires a new `target` section. See [Config File Options](/tools/modelrun#configuration-file) for full details.
  - LLM:
...
- Breaking Changes
  ...
  - Support for VitisAI EP cache encryption is no longer available. To encrypt the compiled models, use the ONNX Runtime [EP Context Cache](/tools/modelrun#ort-ep-context-cache) feature instead.

  - For INT8 models, the VitisAI EP does not save the compiled model to disk by default. To save the compiled model, use the ONNX Runtime [EP Context Cache](/tools/modelrun#ort-ep-context-cache) feature or set the `enable_cache_file_io_in_mem` provider option to 0.

  - Generation of the `vitisai_ep_report.json` file is no longer automatic and should be manually enabled. See the [Operator Assignment Report](/tools/modelrun#op-assignment-report) section for details.
...
- [New BF16 model examples](/vision/index):
...
- New Tools:
  ...
  - [Lemonade SDK](https://github.com/onnx/turnkeyml/blob/main/docs/lemonade/README.md)
    ...
    - [Lemonade Command Line](https://github.com/lemonade-sdk/lemonade) Interface easily benchmark, measure accuracy, prompt or gather memory usage of your LLM.
...
- Improved support for [LLMs using OGA](/llms)
...
- New model support:
  ...
  - [LLM flow support](/llms) for multiple models in both PyTorch and ONNX flow (optimized model support will be released asynchronously)
  - SDXL-T with limited performance optimization

- New EoU tools:

  - [AI Analyzer](/tools/ai_analyzer): Analysis and visualization of model compilation and inference profiling
  - Platform/NPU inspection and management tool ([xrt-smi](/tools/xrt_smi))
  - [Onnx Benchmarking tool](https://github.com/amd/RyzenAI-SW/tree/main/onnx-benchmark)

- New Demos:

  - NPU-GPU multi-model pipeline application [demo](/vision/npu-gpu-pipeline)

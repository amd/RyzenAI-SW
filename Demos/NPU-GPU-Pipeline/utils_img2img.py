...
class Annotator:
    ...
    def __init__(
        ...
        self.original_image = self.im.copy()
    ...
    def crop_object(self, box, save_path=None):
        """
        Crop an object based on the bounding box and optionally save it to a file.
        Args:
            box (tuple): The bounding box with (x1, y1, x2, y2) format.
            save_path (str, optional): Path to save the cropped image. If None, the image is not saved.

        Returns:
            numpy.ndarray: The cropped image array.
        """
        if isinstance(self.original_image, Image.Image):  # If using PIL
            crop_img = self.original_image.crop(box)  # PIL uses (left, upper, right, lower) box format
            if save_path:
                crop_img.save(save_path)
            return np.array(crop_img)
        else:  # If using cv2
            crop_img = self.original_image[box[1]:box[3], box[0]:box[2]]
            if save_path:
        ...
        if len(cls) > 0:
            ...
            for j, box in enumerate(boxes.T.tolist()):
                ...
                if labels or conf[j] > 0.25:  # 0.25 conf thresh
                    ...
                    if save_path:
                        cropped_imgs.append((annotator.crop_object(box, save_path+"detection_{}.jpg".format(j)), c))
                    else: 
                        cropped_imgs.append((annotator.crop_object(box), c))
    return annotator.im, boxes, cropped_imgs, op_labels

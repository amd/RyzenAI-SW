...
if __name__ == "__main__":
    ...
    df = pd.DataFrame(transposed_list, index=Rows, columns=[columns])
    df["[(C-B)/B]*100"] = df.apply(
        lambda row: round(
            (
                (row["power_profile_vitisai.csv"] - row["power_profile_cpu.csv"])
                / row["power_profile_cpu.csv"]
            )
            * 100,
            2,
        )
        if row["power_profile_cpu.csv"] != 0
        else ("nan"),
        axis=1,
    )

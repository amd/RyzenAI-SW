# Hugging Face CPU: meta-llama/CodeLlama-7b-Instruct-hf (bfloat16)

This guide contains all of the instructions necessary to get started with the model [`meta-llama/CodeLlama-7b-Instruct-hf`](https://huggingface.co/meta-llama/CodeLlama-7b-Instruct-hf) on Hugging Face CPU in the bfloat16 data type.
...
```bash
lemonade -i meta-llama/CodeLlama-7b-Instruct-hf huggingface-load --device cpu --dtype bfloat16 llm-prompt --max-new-tokens 64 -p PROMPT
```
...
```bash
lemonade -i meta-llama/CodeLlama-7b-Instruct-hf huggingface-load --device cpu --dtype bfloat16 huggingface-bench --warmup-iterations 5 --iterations 10 --prompt 1024 --output-tokens 64
```
...
model, tokenizer = from_pretrained(
    "meta-llama/CodeLlama-7b-Instruct-hf", recipe="hf-cpu"
)
...
model, tokenizer = from_pretrained("meta-llama/CodeLlama-7b-Instruct-hf", recipe="hf-cpu")

# Hugging Face CPU: Qwen/Qwen2-7B (bfloat16)

This guide contains all of the instructions necessary to get started with the model [`Qwen/Qwen2-7B`](https://huggingface.co/Qwen/Qwen2-7B) on Hugging Face CPU in the bfloat16 data type.

The CPU implementation in this guide is designed to run on most PCs. However, for optimal performance on Ryzen AI 300-series PCs, try the [hybrid execution mode](../hybrid/Qwen2_7B.md).
...
The Hugging Face CPU implementation of [`Qwen/Qwen2-7B`](https://huggingface.co/Qwen/Qwen2-7B) uses the original safetensors weights provided in the Hugging Face checkpoint.
...
```bash
lemonade -i Qwen/Qwen2-7B huggingface-load --device cpu --dtype bfloat16 llm-prompt --max-new-tokens 64 -p PROMPT
```
...
```bash
lemonade -i Qwen/Qwen2-7B huggingface-load --device cpu --dtype bfloat16 huggingface-bench --warmup-iterations 5 --iterations 10 --prompt 1024 --output-tokens 64
```
...
```bash
lemonade -i Qwen/Qwen2-7B huggingface-load --device cpu --dtype bfloat16 accuracy-mmlu --tests management
```
...
model, tokenizer = from_pretrained("Qwen/Qwen2-7B", recipe="hf-cpu")

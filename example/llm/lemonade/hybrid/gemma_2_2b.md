# Ryzen AI Hybrid: google/gemma-2-2b (int4)

This guide contains all of the instructions necessary to get started with the model [`google/gemma-2-2b`](https://huggingface.co/google/gemma-2-2b) on Ryzen AI Hybrid in the int4 data type.
...
The Ryzen AI Hybrid implementation of [`google/gemma-2-2b`](https://huggingface.co/google/gemma-2-2b) uses the [`amd/gemma-2-2b-awq-uint4-asym-g128-lmhead-g32-fp16-onnx-hybrid`](https://huggingface.co/amd/gemma-2-2b-awq-uint4-asym-g128-lmhead-g32-fp16-onnx-hybrid) checkpoint on Hugging Face. This checkpoint has been quantized for int4 with AMD Quark using the recipe provided on the model card.
...
```bash
lemonade -i amd/gemma-2-2b-awq-uint4-asym-g128-lmhead-g32-fp16-onnx-hybrid oga-load --device hybrid --dtype int4 llm-prompt --max-new-tokens 64 -p PROMPT
```
...
```bash
lemonade -i amd/gemma-2-2b-awq-uint4-asym-g128-lmhead-g32-fp16-onnx-hybrid oga-load --device hybrid --dtype int4 oga-bench --warmup-iterations 5 --iterations 10 --prompt 1024 --output-tokens 64
```
...
```bash
lemonade -i amd/gemma-2-2b-awq-uint4-asym-g128-lmhead-g32-fp16-onnx-hybrid oga-load --device hybrid --dtype int4 accuracy-mmlu --tests management
```
...
model, tokenizer = from_pretrained(
    "amd/gemma-2-2b-awq-uint4-asym-g128-lmhead-g32-fp16-onnx-hybrid", recipe="oga-hybrid"
)

...
class WhisperONNX:
    def __init__(self, encoder_path, decoder_path,
        ...
        self.feature_extractor = WhisperFeatureExtractor.from_pretrained(tokenizer_dir)
...
def load_provider_options(device):
    if device == "cpu":
        ...
        return [(provider, encoder_options)], [(provider, decoder_options)]
...
def mic_stream(model, duration=0, silence_threshold=0.01, silence_duration=5.0):
    ...
    def feeder():
        try:
            ...
            print(f"\n Microphone error: {e}")
            print("Could not initialize microphone. Please check your audio device settings.")
            stop_flag.set()
...
def main():
    ...
    encoder_providers, decoder_providers = load_provider_options(args.device
    )

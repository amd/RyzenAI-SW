...
tv = tv.split(".")
if int(tv[1])>=39:
    from transformers.models.gemma.modeling_gemma import GemmaRMSNorm, GemmaDecoderLayer
...
def auto_scale_block(module, module_kwargs, w_bit, q_config, input_feat):
    ...
    def _search_module_scale(block, linears2scale: list, x, kwargs={}):
        ...
        for ratio in range(n_grid):
            ...
            #print(f"_search_module_scale: {ratio} {n_grid} {kwargs.keys()} {x.shape}")
            out = block(x, **kwargs)
    ...
    elif module.__class__.__name__ == "QWenBlock":
        # attention input
        scales_list.append(_auto_get_scale(
            prev_op=module.ln_1,
...
def apply_scale(module, scales_list, input_feat_dict=None):
    for prev_op_name, layer_names, scales in scales_list:
        ...
        elif prev_op.__class__.__name__ == "LlamaRMSNorm":
            scale_ln_fcs(prev_op, layers, scales)
        elif prev_op.__class__.__name__ == "Qwen2RMSNorm":
            scale_ln_fcs(prev_op, layers, scales)
        elif prev_op.__class__.__name__ == "RMSNorm":
            scale_ln_fcs(prev_op, layers, scales)
        elif prev_op.__class__.__name__ == "GemmaRMSNorm":
            scale_ln_fcs(prev_op, layers, scales)

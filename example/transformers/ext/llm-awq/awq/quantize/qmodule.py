...
class WQLinear(nn.Module):
    def __init__(self, w_bit, group_size, in_features, out_features, bias, dev):
        ...
        self.register_buffer('qweight', torch.zeros((out_features, in_features // pack_num), dtype=torch.int8, device=dev))
        self.register_buffer('qzeros', torch.zeros((out_features, calculate_zeros_width(in_features, self.group_size)), dtype=torch.int8, device=dev))
        self.register_buffer('scales', torch.zeros((out_features, calculate_zeros_width(in_features, self.group_size) * pack_num), dtype=torch.bfloat16, device=dev))
        if bias:
            self.register_buffer('bias', torch.zeros((out_features), dtype=torch.bfloat16, device=dev))
        else:
            self.bias = None
    ...
    def from_linear(cls, linear, w_bit, group_size, init_only=False, scales=None, zeros=None):
        ...
        if False: # disable packing 
            ...
            awq_linear.qzeros = zeros
        awq_linear.qzeros = awq_linear.qzeros.to(torch.int8)
        awq_linear.qweight = awq_linear.qweight.to(torch.int8)
        return awq_linear

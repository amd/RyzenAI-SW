...
std::tuple<int, int> silu_qdq<InT, WtT, OutT>::map_padded_shape(int M, int N) {
...
template <typename InT, typename WtT, typename OutT>
std::string silu_qdq<InT, WtT, OutT>::get_instr_key(std::string prefix, int m,
                                                    int k) {
...
silu_qdq<InT, WtT, OutT>::silu_qdq(
  ...
  default_shapes_["silu_qdq_4x2_abf16acc16"] =
      std::vector<std::tuple<int, int>>();

  default_shapes_["silu_qdq_4x2_abf16acc16"].push_back(
...
void silu_qdq<InT, WtT, OutT>::initialize_const_params(
  ...
  DOD_THROW_IF((const_params.size() != 1) ||
               ...
               OpsFusion::dod_format("Unsupported const spec for Silu_qdq\n") +
  ...
  memcpy((void *)(static_cast<int8_t *>(dest)), (void *)silu_lutab,
         silu_qdq_size_lutab);
  memcpy((void *)(static_cast<int8_t *>(dest) + silu_qdq_size_lutab),
         (void *)silu_lutcd, silu_qdq_size_lutcd);
...
template <typename InT, typename WtT, typename OutT>
void silu_qdq<InT, WtT, OutT>::execute(const std::vector<Tensor> &input,

...
bool mladf_add<LhsT, RhsT, OutT>::isSupportedShape(const Tensor &operand) {
...
template <typename LhsT, typename RhsT, typename OutT>
mladf_add<LhsT, RhsT, OutT>::mladf_add(const std::string &operand_dtype,
                                       bool load_xrt) {
  ...
  default_shapes_["mladfadd_a16a16"] = std::vector<std::tuple<int, int>>();
  default_shapes_["mladfadd_a16a16"].push_back(std::make_tuple(4096, 4096));
  ...
  txn_fname_prefix_ = "mladfadd_" + txnbin_operand_header.at(operand_dtype_) +
  ...
  if (load_xrt) {
    ...
    const size_t operand_size_in_bytes =
        max_pairwise_product(default_shapes_["mladfadd_a16a16"]) *
        operand_dtype_size_;
  ...
  std::call_once(logger_flag_, []() {
    std::string header = "mladf_add_id M K N kernel_m kernel_k kernel_n Execute"
...
template <typename LhsT, typename RhsT, typename OutT>
void mladf_add<LhsT, RhsT, OutT>::execute(const std::vector<Tensor> &input,
  ...
  RYZENAI_LOG_INFO(
      std::to_string(mladf_add_id_) + " " + std::to_string(operand_shape_[0]) +

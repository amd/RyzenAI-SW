...
template <typename InT, typename WtT, typename OutT>
std::string mhawindow<InT, WtT, OutT>::get_instr_key(std::string prefix,
...
void mhawindow<InT, WtT, OutT>::initialize_const_params(
  ...
  DOD_ASSERT((const_params.size() == 1),
             OpsFusion::dod_format("MHAWINDOW expects one constant. Got {}",
                                   const_params.size()));
...
// Q+K+V
template <typename InT, typename WtT, typename OutT>
void mhawindow<InT, WtT, OutT>::execute(const std::vector<Tensor> &input,

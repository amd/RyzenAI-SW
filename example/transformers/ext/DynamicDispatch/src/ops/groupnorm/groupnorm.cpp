...
namespace ryzenai {
static std::tuple<int, int, int> extract_MK(const std::vector<Tensor> &inputs) {
  int M = 0;
  int N = 0;
  int G = 0;
  if (inputs.at(0).shape.size() == 4) {                      // NHWC format PSR
...
std::tuple<int, int> groupnorm<InT, WtT, OutT>::map_padded_shape(int M, int N) {
...
template <typename InT, typename WtT, typename OutT>
std::string groupnorm<InT, WtT, OutT>::get_instr_key(std::string prefix, int m,
                                                     int k, int g) {
...
std::vector<OpArgMap> groupnorm<InT, WtT, OutT>::get_buffer_reqs(
  ...
  size_t const_params_bo_size = ((gamma_dim + beta_dim) * sizeof(WtT) * 8 +
...
groupnorm<InT, WtT, OutT>::groupnorm(
  ...
  default_shapes_["gpn_4x2_abf16accbf16"] =
      std::vector<std::tuple<int, int, int>>();
  default_shapes_["gpn_4x2_abf16accbf16"].push_back(
  ...
  // default shape
  raw_shapes_["gpn_4x2_abf16accbf16"] =
      std::vector<std::tuple<int, int, int>>();
  raw_shapes_["gpn_4x2_abf16accbf16"].push_back(
      std::make_tuple(64, 1280, 1280));
  raw_shapes_["gpn_4x2_abf16accbf16"].push_back(
      std::make_tuple(64, 2560, 2560));
  raw_shapes_["gpn_4x2_abf16accbf16"].push_back(
  ...
  raw_shapes_["gpn_4x2_abf16accbf16"].push_back(
      std::make_tuple(1024, 320, 320));
  raw_shapes_["gpn_4x2_abf16accbf16"].push_back(std::make_tuple(256, 640, 640));
...
void groupnorm<InT, WtT, OutT>::initialize_const_params(
  ...
  //              OpsFusion::dod_format("Unsupported const spec for
  //              Groupnorm\n") +
  ...
  // Group Norm kernel requires Gamma and Beta to be repeated 8 times
  lrn_matrix::BiasVector<WtT, 1> bias(8 * shape[0], w_dest);
  for (size_t k = 0; k < 8 * shape[0]; ++k) {
    bias.gamma(k) = gamma[k % shape[0]];
    bias.beta(k) = beta[k % shape[0]];
  }
  auto offset = shape[0] * 8 * 2;
...
void groupnorm<InT, WtT, OutT>::initialize_const_params(
  ...
  kernel_y_shape_[0] = 8 * 2; // Bo has Gamma and Beta repeated 8 times
...
template <typename InT, typename WtT, typename OutT>
void groupnorm<InT, WtT, OutT>::execute(const std::vector<Tensor> &input,

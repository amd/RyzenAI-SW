...
void mha<InT, WtT, OutT>::initialize_const_params(
  ...
  qdq_param[(16 * 2) + qdq_Mv_idx] = mha_psq2_st_pad;
  qdq_param[(16 * 2) + qdq_Nv_idx] = mha_psq2_st_pad;

  qdq_param[(16 * 3) + qdq_Mv_idx] = mha_psq2_st_pad;
  qdq_param[(16 * 3) + qdq_Nv_idx] = mha_psq2_val_subv_cols;
...
void mha<InT, WtT, OutT>::initialize_const_params(
  ...
  DOD_ASSERT((const_params.size() == 1),
             OpsFusion::dod_format("mha expects one constant. Got {}",
                                   const_params.size()));
...
template <typename InT, typename WtT, typename OutT>
void mha<InT, WtT, OutT>::execute(const std::vector<Tensor> &input,

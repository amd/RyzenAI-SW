...
std::tuple<size_t, size_t> mhachannel<InT, WtT, OutT>::map_padded_shape(int M,
                                                                        int N) {
...
void mhachannel<InT, WtT, OutT>::initialize_const_params(
  ...
  DOD_ASSERT((const_params.size() == 1),
             OpsFusion::dod_format("MHACHANNEL expects one constant. Got {}",
                                   const_params.size()));
...
// Q+K+V
template <typename InT, typename WtT, typename OutT>
void mhachannel<InT, WtT, OutT>::execute(const std::vector<Tensor> &input,

...
int test_mhawindow(int M, int K, int N, bool debug = false,
  ...
  *(int64_t *)(&qdq_params[(16 * 3) + qdq_c0_idx]) = smv_qdq_c0;
  ...
  std::string fld_name;
  if (M == 1) {
    fld_name = "//bin_files//PSI_MHAWIN//data_block3//";
  } else if (M == 4) {
    fld_name = "//bin_files//PSI_MHAWIN//data_block2//";
  } else if (M == 16) {
    fld_name = "//bin_files//PSI_MHAWIN//data_block1//";
  } else {
    fld_name = "//bin_files//PSI_MHAWIN//data_block0//";
  }
...
// MHAWINDOW
TEST(PSI_MHAWINDOW_Testa16w8, Kernel1) {
  int err_count = test_mhawindow<uint16_t, uint8_t, uint16_t>(
      64, 49, 384, false, "uint16", "uint8", "uint16", "PSI");
  EXPECT_TRUE(err_count == 0) << "Error Count = " << err_count;
}

TEST(PSI_MHAWINDOW_Testa16w8, Kernel2) {
  int err_count = test_mhawindow<uint16_t, uint8_t, uint16_t>(
      16, 49, 768, false, "uint16", "uint8", "uint16", "PSI");
  EXPECT_TRUE(err_count == 0) << "Error Count = " << err_count;
}

TEST(PSI_MHAWINDOW_Testa16w8, Kernel3) {
  int err_count = test_mhawindow<uint16_t, uint8_t, uint16_t>(
      4, 49, 1536, false, "uint16", "uint8", "uint16", "PSI");
  EXPECT_TRUE(err_count == 0) << "Error Count = " << err_count;
}

TEST(PSI_MHAWINDOW_Testa16w8, Kernel4) {
  int err_count = test_mhawindow<uint16_t, uint8_t, uint16_t>(

...
int test_mhapsr(int M, int K, int N, bool debug = false,
  ...
  batch = 1;
  heads = 1; // each head is processed separately in PSR
  x = Ks;
  y = 64;
  ...
  *(int64_t *)(&qdq_params[(16 * 3) + qdq_c0_idx]) = smv_qdq_c0;
  ...
  std::string fld_name;
  if (M == 64) {
    fld_name = "//bin_files//PSR_mha//cross_m64//";
  } else if (M == 256) {
    fld_name = "//bin_files//PSR_mha//cross_m256//";
  } else if (M == 1024) {
    fld_name = "//bin_files//PSR_mha//cross_m1024//";
  } else {
    fld_name = "//bin_files//PSR_mha//cross_m4096//";
  }

  if (K == 64) {
    fld_name = "//bin_files//PSR_mha//self_m64//";
  } else if (K == 256) {
    fld_name = "//bin_files//PSR_mha//self_m256//";
  } else if (K == 1024) {
    fld_name = "//bin_files//PSR_mha//self_m1024//";
  } else {
    fld_name = "//bin_files//PSR_mha//self_m4096//";
  }
  ...
  *(int64_t *)(&qdq_params[(16 * 3) + 0]) = 43320113889280;
  ...
  ryzenai::mhapsr mhapsr_ =
      ryzenai::mhapsr<Tqkv, WgT, OuT>(a_dtype, b_dtype, c_dtype, false, attr);
  mhapsr_.debug(debug);
  // std::vector<size_t> a_shape = {qry_rows, qry_cols, key_rows, val_cols};
  mhapsr_.set_params(model_name, qkv_shape);

  std::vector<Tensor> const_Tensor;

  const_Tensor = {{qdq_params.data(), qdq_shape, "int32"}};
  mhapsr_.initialize_const_params(const_Tensor);
  ...
  PROFILE_THIS(mhapsr_.execute(input_Tensor, output_Tensor));
#else
  mhapsr_.execute(input_Tensor, output_Tensor);
#endif
...
// mhapsr cross PSR 4x2
TEST(PSR_mhapsr_Testa16w8, Kernel1) {
  int err_count = test_mhapsr<uint16_t, uint8_t, uint16_t>(
      64, 77, 64, false, "uint16", "uint8", "uint16", "PSR");
  EXPECT_TRUE(err_count == 0) << "Error Count = " << err_count;
}

TEST(PSR_mhapsr_Testa16w8, Kernel2) {
  int err_count = test_mhapsr<uint16_t, uint8_t, uint16_t>(
      256, 77, 64, false, "uint16", "uint8", "uint16", "PSR");
  EXPECT_TRUE(err_count == 0) << "Error Count = " << err_count;
}

TEST(PSR_mhapsr_Testa16w8, Kernel3) {
  int err_count = test_mhapsr<uint16_t, uint8_t, uint16_t>(
      1024, 77, 64, false, "uint16", "uint8", "uint16", "PSR");
  EXPECT_TRUE(err_count == 0) << "Error Count = " << err_count;
}

TEST(PSR_mhapsr_Testa16w8, Kernel4) {
  int err_count = test_mhapsr<uint16_t, uint8_t, uint16_t>(
      4096, 77, 64, false, "uint16", "uint8", "uint16", "PSR");
  EXPECT_TRUE(err_count == 0) << "Error Count = " << err_count;
}

// mhapsr self PSR 4x2
TEST(PSR_mhapsr_Testa16w8, Kernel5) {
  int err_count = test_mhapsr<uint16_t, uint8_t, uint16_t>(
      64, 64, 64, false, "uint16", "uint8", "uint16", "PSR");
  EXPECT_TRUE(err_count == 0) << "Error Count = " << err_count;
}

// mhapsr cross PSR 4x4
TEST(C4PSR_mhapsr_Testa16w8, Kernel1) {
  int err_count = test_mhapsr<uint16_t, uint8_t, uint16_t>(
      64, 77, 64, false, "uint16", "uint8", "uint16", "4x4PSR");
  EXPECT_TRUE(err_count == 0) << "Error Count = " << err_count;
}

TEST(C4PSR_mhapsr_Testa16w8, Kernel2) {
  int err_count = test_mhapsr<uint16_t, uint8_t, uint16_t>(
      256, 77, 64, false, "uint16", "uint8", "uint16", "4x4PSR");
  EXPECT_TRUE(err_count == 0) << "Error Count = " << err_count;
}

TEST(C4PSR_mhapsr_Testa16w8, Kernel3) {
  int err_count = test_mhapsr<uint16_t, uint8_t, uint16_t>(
      1024, 77, 64, false, "uint16", "uint8", "uint16", "4x4PSR");
  EXPECT_TRUE(err_count == 0) << "Error Count = " << err_count;
}

TEST(C4PSR_mhapsr_Testa16w8, Kernel4) {
  int err_count = test_mhapsr<uint16_t, uint8_t, uint16_t>(
      4096, 77, 64, false, "uint16", "uint8", "uint16", "4x4PSR");
  EXPECT_TRUE(err_count == 0) << "Error Count = " << err_count;
}

// mhapsr self PSR 4x4
TEST(C4PSR_mhapsr_Testa16w8, Kernel5) {
  int err_count = test_mhapsr<uint16_t, uint8_t, uint16_t>(
      64, 64, 64, false, "uint16", "uint8", "uint16", "4x4PSR");
  EXPECT_TRUE(err_count == 0) << "Error Count = " << err_count;
}

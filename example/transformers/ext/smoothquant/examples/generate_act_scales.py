...
def build_model_and_tokenizer(model_name):
    if model_name == "facebook/llama2-7b":
        tokenizer = LlamaTokenizer.from_pretrained("../llama-2-wts-hf/7B_orig")
        model = LlamaForCausalLM.from_pretrained("../llama-2-wts-hf/7B_orig")
    elif model_name == "facebook/llama2-7b-chat":
        tokenizer = LlamaTokenizer.from_pretrained("../llama-2-wts-hf/7B_chat")
        model = LlamaForCausalLM.from_pretrained("../llama-2-wts-hf/7B_chat")
    else:
        tokenizer = AutoTokenizer.from_pretrained(model_name, model_max_length=512)
        kwargs = {"torch_dtype": torch.float, "device_map": "sequential"}

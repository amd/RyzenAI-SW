...
def make_matmultest(model_name, M, K, N):
    ...
    dynamic_quant = onnx.helper.make_node(
        name="/custom/DynamicQuantizeLinear",
        op_type="DynamicQuantizeLinear",
        inputs=["X"],
        outputs=["Y", "Y_SCALE", "Y_ZP"],
    )

    w11 = np.random.randint(low=-128, high=127, size=(K, N))
    W11 = onnx.helper.make_tensor(
        name="W11",
        data_type=TensorProto.INT8,
        dims=w11.shape,
        vals=w11.flatten().tolist(),
    )
    w11_zp = np.array([0])
    W11_ZP = onnx.helper.make_tensor(
        name="W11_ZP",
        data_type=TensorProto.INT8,
        dims=w11_zp.shape,
        vals=w11_zp.flatten().tolist(),
    )
    Z1 = onnx.helper.make_tensor_value_info("Z1", TensorProto.INT32, [M, N])
    matmul1 = onnx.helper.make_node(
        name="/custom/MatMulInteger1",
        op_type="MatMulInteger",
        inputs=["Y", W11.name, "Y_ZP", W11_ZP.name],
        outputs=["Z1"],
    )

    w12 = np.random.randint(low=-128, high=127, size=(K, N))
    W12 = onnx.helper.make_tensor(
        name="W12",
        data_type=TensorProto.INT8,
        dims=w12.shape,
        vals=w12.flatten().tolist(),
    )
    w12_zp = np.array([0])
    W12_ZP = onnx.helper.make_tensor(
        name="W12_ZP",
        data_type=TensorProto.INT8,
        dims=w12_zp.shape,
        vals=w12_zp.flatten().tolist(),
    )
    Z2 = onnx.helper.make_tensor_value_info("Z2", TensorProto.INT32, [M, N])
    matmul2 = onnx.helper.make_node(
        name="/custom/MatMulInteger2",
        op_type="MatMulInteger",
        inputs=["Y", W12.name, "Y_ZP", W12_ZP.name],
        outputs=["Z2"],
    )

    w13 = np.random.randint(low=-128, high=127, size=(K, N))
    W13 = onnx.helper.make_tensor(
        name="W13",
        data_type=TensorProto.INT8,
        dims=w13.shape,
        vals=w13.flatten().tolist(),
    )
    w13_zp = np.array([0])
    W13_ZP = onnx.helper.make_tensor(
        name="W13_ZP",
        data_type=TensorProto.INT8,
        dims=w13_zp.shape,
        vals=w13_zp.flatten().tolist(),
    )
    Z3 = onnx.helper.make_tensor_value_info("Z3", TensorProto.INT32, [M, N])
    matmul3 = onnx.helper.make_node(
        name="/custom/MatMulInteger3",
        op_type="MatMulInteger",
        inputs=["Y", W13.name, "Y_ZP", W13_ZP.name],
        outputs=["Z3"],
    )

    graph = onnx.helper.make_graph(
        [dynamic_quant, matmul1, matmul2, matmul3],
        "Custom",
        inputs=[X],
        outputs=[Z1, Z2, Z3],
        initializer=[W11, W11_ZP, W12, W12_ZP, W13, W13_ZP],
    )

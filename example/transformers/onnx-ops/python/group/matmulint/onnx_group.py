...
class GroupMatMulInteger:
    ...
    def replace(self, dynamicquant, matmulint_list):
        ...
        weight_concat_tensor = numpy_helper.from_array(
            weight_concat, name=weight_concat_name
        )
        new_inp = [
            dynamicquant.output[0],
            weight_concat_tensor.name,
            dynamicquant.output[2],
            matmulint_list[0].input[3],
        ]
        ...
        new_matmul = onnx.helper.make_node(
            name=op_name,
            op_type=GroupMatMulInteger.op_type,
            inputs=new_inp,
            outputs=[op_out_name],
        )

        split_op_name = op_out_name + "_Split"
        split_dim_tensor = numpy_helper.from_array(
            np.array(split_dim).astype(np.int64), name=split_op_name + "/Dim"
        )
        split_op = onnx.helper.make_node(
            name=split_op_name,
            op_type="Split",
            inputs=[op_out_name, split_dim_tensor.name],
            outputs=outv,
            axis=-1,
        )
        ...
        self.onnx_graph.model.graph.initializer.extend(
            [weight_concat_tensor, split_dim_tensor]
        )
    ...
    def group(self):
        ...
        dynamicquantlinear = [
            node
            for node in self.onnx_graph.nodev.values()
            if (node.op != None) and (node.op.op_type == "DynamicQuantizeLinear")
        ]
        for d in dynamicquantlinear:
            ...
            for i in range(3):
                ...
                if (
                    (matmul != None)
                    and (matmul.op != None)
                    and (matmul.op.op_type == "MatMulInteger")
                ):
    ...
    def save(self, output):
        self.onnx_graph.model.opset_import.append(
            onnx.helper.make_opsetid("ryzenai.customop", 1)
        )

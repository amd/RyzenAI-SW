...
class ExprFunctor<R(const PrimExpr& n, Args...)> {
 ...
 public:
  ...
  virtual R VisitExpr_(const NoneNode* op, Args... args) EXPR_FUNCTOR_DEFAULT;
 ...
 private:
  ...
  static FType InitVTable() {
    ...
    IR_EXPR_FUNCTOR_DISPATCH(NoneNode);
    return vtable;
  }
};
...
class TVM_DLL ExprVisitor : public ExprFunctor<void(const PrimExpr&)> {
 ...
 protected:
  ...
  void VisitExpr_(const NoneNode* op) override;
};
...
class TVM_DLL ExprMutator : protected ExprFunctor<PrimExpr(const PrimExpr&)> {
 ...
 protected:
  ...
  PrimExpr VisitExpr_(const NoneNode* op) override;
};

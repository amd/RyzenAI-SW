...
class Stage : public ObjectRef {
 public:
  ...
  /*!
   * \brief Compute the complete range for the specified iteration variables in this stage.
   *  No boundary inference optimization will be done on these iteration variables during the InferBound step.
   * \return reference to self.
   */
  TVM_DLL Stage& compute_all(const Array<IterVar>& ivars);  // NOLINT(*)
  /*!
...
class StageNode : public Object {
 public:
  ...
  /*! \brief The iteration variables that have to be computed over the complete range */
  Array<IterVar> compute_all;
  ...
  void VisitAttrs(AttrVisitor* v) {
    ...
    v->Visit("compute_all", &compute_all);
  }

...
/*!
 * \brief Padding the inputs of operators in order to offload this op to Aie.
 * \param expr The original expr.
 * \param post The modified expr in AiePadding Pass.
 * \return New expr with padded input shapes.
 */
using FTVMAiePadding = runtime::TypedPackedFunc<Expr(const Expr& pre, const Expr& post)>;

...
class RotaryEmbedding(nn.Module):
    def __init__(
        ...
        self.base = base
        # (inv_freq) $\Theta = {\theta_i = 10000^{\frac{2(i-1)}{d}}, i \in [1, 2, ..., \frac{d}{2}]}$
        inv_freq = 1.0 / (
            self.base
            ** (torch.arange(0, self.dim, 2, device=device).to(dtype=dtype) / self.dim)
        )
    ...
    def _set_cos_sin_cache(
        ...
        idx_theta = torch.outer(seq_idx, self.inv_freq).float()
        ...
        self.register_buffer("cos_sin_cached", cache, persistent=False)

    def forward(self, seq_len, position_ids=None):
        ...
        if position_ids is not None:
            rotary_pos_emb = self.cos_sin_cached[position_ids]
        else:
            rotary_pos_emb = self.cos_sin_cached[None, :seq_len]
        return rotary_pos_emb.transpose(0, 1).contiguous()  # [sq, b, head_dim // 2, 2]
...
def apply_rotary_pos_emb(x: torch.Tensor, rope_cache: torch.Tensor) -> torch.Tensor:
    # x: [sq, b, np, hn]
    sq, b, np, _ = x.shape
    rot_dim = rope_cache.shape[-2]
    x1, x_pass = x[..., : (rot_dim * 2)], x[..., (rot_dim * 2) :]
    x1 = x1.view(sq, b, np, -1, 2)

    # truncate to support variable sizes
    rc = rope_cache[:sq].unsqueeze(1)
    cos = rc[..., 0]
    sin = rc[..., 1]
...
class CoreAttention(torch.nn.Module):
    ...
    def forward(self, query_layer, key_layer, value_layer, attention_mask):
        ...
        b, _, sq, _ = query_layer.shape
        _, _, sk, _ = key_layer.shape
        ...
        if pytorch_major_version >= 2:
            ...
            context_layer = torch.nn.functional.scaled_dot_product_attention(
                ...
                is_causal=(attention_mask is None and sq == sk),
            )
        else:
            ...
            if attention_mask is None and sq == sk:
                attention_mask = torch.ones(
                    b, 1, sq, sk, device=attention_scores.device, dtype=torch.bool
                )
            ...
            attention_probs = F.softmax(attention_scores, dim=-1).to(value_layer.dtype)
...
class SelfAttention(torch.nn.Module):
    ...
    def __init__(self, config: ChatGLMConfig, layer_number, device=None):
        ...
        self.num_multi_query_groups_per_partition = config.num_attention_heads
...
class ChatGLMModel(ChatGLMPreTrainedModel):
    ...
    def forward(
        ...
        # Rotary positional embeddings
        rotary_pos_emb = self.rotary_pos_emb(seq_length, position_ids=position_ids)

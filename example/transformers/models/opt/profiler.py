        ...
        for i, line in enumerate(logf):
            ...
            if len(line) > 4:
                ...
                if (line[4] == "generate:"):
                    ...
                    num_tokens_in = int(line[10])
                    cls.num_tokens_in_arr.append(num_tokens_in)
        print(f"Number of prompts found in log: {len(cls.start_idx_arr)}")

    @classmethod 
    def parse_section(cls, outlog, filename, prompt_num, logf, start_idx, end_idx, generate_time, num_tokens_out, num_tokens_in, ops_profiler, amdopt):
        cls.exec_time_arr = []
        cls.prompt_time_arr = []
        cls.token_time_arr = []
        cls.decoder_time_arr = []
        ...
        for i in range(start_idx, end_idx, 1):
            ...
            if line[4] != "model_decoder_forward":
                ...
                k = int(line[5])
                n = int(line[6])
                exec_time_start = int(line[14])
                exec_time_end = int(line[15])
            ...
            if line[4] == "model_decoder_forward":
                #print(line)
                decoder_time = int(line[5])
                pre_processing_time = int(line[6])
                post_processing_time = int(line[7])
        ...
        if amdopt is False:
            outlog.write(f"{filename},{prompt_num+1},{num_tokens_in},{generate_time*1e-9:.3f},na,na,na,na,na,na,{num_tokens_out},{new_tokens_generated},na,na,na\n")
        else:
            outlog.write(f"{filename},{prompt_num+1},{num_tokens_in},{generate_time*1e-9:.3f},{pre_processing_time_prefill_phase*1e-9:.3f},{decoder_time_prefill_phase*1e-9:.3f},{post_processing_time_prefill_phase*1e-9:.3f},{pre_processing_time_token_phase*1e-9:.3f},{decoder_time_token_phase*1e-9:.3f},{post_processing_time_token_phase*1e-9:.3f},{num_tokens_out},{new_tokens_generated},{prefill_phase:.0f},{time_per_token:.0f},{1000.0/time_per_token:.1f}\n")
        outlog.flush()
...
if __name__ == "__main__":
    in_file = "./logs_opt-1.3b/log_opt-1.3b_aie_ptdq.log"
    out_file = in_file.replace(".log", "_profile.csv")
    out_file = open(out_file, "w")
    ProfileAIE.analyze_profiling(True, False, in_file, out_file)
    out_file.close()
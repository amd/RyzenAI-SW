 ...
 * M x N is the actual mul shape which must be linearized
 *
 */
template <typename InOutT = int16_t>
int test_mul(int M, int N, bool debug = false,
  ...
  assert(a_dtype == "bfloat16" &&
         "Currently only supporting bfloat16 -> mul -> bfloat16");
  ...
  initialize_random<InOutT>(b, M * N, 42, "bfloat16");

  // compute goldens
  for (int i = 0; i < M * N; ++i) {
    float lhs = ryzenai::bfloat16_to_float(a[i]);
    float rhs = ryzenai::bfloat16_to_float(b[i]);
    float result = lhs * rhs;

    c_golden[i] = ryzenai::float_to_bfloat16(result);
  }

  ryzenai::mul mulKernel = ryzenai::mul<InOutT>(a_dtype, b_dtype, c_dtype);

  mulKernel.debug(debug);
  for (int i = 0; i < stoi(NUM_EXECUTE_ITERATIONS_); i++)
    mulKernel.execute(a.data(), b.data(), c.data(), a_shape);
  ...
  for (int i = 0; i < c.size(); i++) {
    ...
    if (err > EPSILON && false) {
      std::cout << "a[" << i << "]: " << ryzenai::bfloat16_to_float(a[i])
                << "\n";
      std::cout << "b[" << i << "]: " << ryzenai::bfloat16_to_float(b[i])
                << "\n";
      std::cout << "c[" << i << "]: "
...
/*Llama2 shapes tests*/
TEST(mulNoPadding, Kernel_1x11008) {
  if (std::string((Utils::get_env_var("DEVICE"))) == "stx") {
    int err_count =
        test_mul<int16_t>(1, 11008, false, "bfloat16", "bfloat16", "bfloat16");
...
TEST(mulNoPadding, Kernel_128x11008) {
  if (std::string((Utils::get_env_var("DEVICE"))) == "stx") {
    int err_count = test_mul<int16_t>(128, 11008, false, "bfloat16", "bfloat16",
                                      "bfloat16");
    EXPECT_TRUE(err_count == 0) << "Error Count = " << err_count;
  } else {
    GTEST_SKIP() << "Test not supported on " << Utils::get_env_var("DEVICE");
...
TEST(mulNoPadding, Kernel_256x11008) {
  if (std::string((Utils::get_env_var("DEVICE"))) == "stx") {
    int err_count = test_mul<int16_t>(256, 11008, false, "bfloat16", "bfloat16",
                                      "bfloat16");
    EXPECT_TRUE(err_count == 0) << "Error Count = " << err_count;
  } else {
    GTEST_SKIP() << "Test not supported on " << Utils::get_env_var("DEVICE");
...
TEST(mulNoPadding, Kernel_512x11008) {
  if (std::string((Utils::get_env_var("DEVICE"))) == "stx") {
    int err_count = test_mul<int16_t>(512, 11008, false, "bfloat16", "bfloat16",
                                      "bfloat16");
    EXPECT_TRUE(err_count == 0) << "Error Count = " << err_count;
  } else {
    GTEST_SKIP() << "Test not supported on " << Utils::get_env_var("DEVICE");
...
TEST(mulNoPadding, Kernel_800x11008) {
  if (std::string((Utils::get_env_var("DEVICE"))) == "stx") {
    // Not supported yet
    EXPECT_THROW(std::ignore = test_mul<int16_t>(800, 11008, false, "bfloat16",
                                                 "bfloat16", "bfloat16"),
                 std::runtime_error);
  } else {
    GTEST_SKIP() << "Test not supported on " << Utils::get_env_var("DEVICE");
...
TEST(mulNoPadding, Kernel_1024x11008) {
  if (std::string((Utils::get_env_var("DEVICE"))) == "stx") {
    int err_count = test_mul<int16_t>(1024, 11008, false, "bfloat16",
                                      "bfloat16", "bfloat16");
    EXPECT_TRUE(err_count == 0) << "Error Count = " << err_count;
  } else {
    GTEST_SKIP() << "Test not supported on " << Utils::get_env_var("DEVICE");
...
TEST(mulNoPadding, Kernel_2000x11008) {
  if (std::string((Utils::get_env_var("DEVICE"))) == "stx") {
    // Not supported yet
    EXPECT_THROW(std::ignore = test_mul<int16_t>(2000, 11008, false, "bfloat16",
                                                 "bfloat16", "bfloat16"),
                 std::runtime_error);
  } else {
    GTEST_SKIP() << "Test not supported on " << Utils::get_env_var("DEVICE");
...
TEST(mulNoPadding, Kernel_2048x11008) {
  if (std::string((Utils::get_env_var("DEVICE"))) == "stx") {
    int err_count = test_mul<int16_t>(2048, 11008, false, "bfloat16",
                                      "bfloat16", "bfloat16");
    EXPECT_TRUE(err_count == 0) << "Error Count = " << err_count;
  } else {
    GTEST_SKIP() << "Test not supported on " << Utils::get_env_var("DEVICE");
...
TEST(mulNoPadding, Kernel_123x11008) {
  if (std::string((Utils::get_env_var("DEVICE"))) == "stx") {
    // Not supported yet
    EXPECT_THROW(std::ignore = test_mul<int16_t>(123, 11008, false, "bfloat16",
                                                 "bfloat16", "bfloat16"),
                 std::runtime_error);
  } else {
    GTEST_SKIP() << "Test not supported on " << Utils::get_env_var("DEVICE");

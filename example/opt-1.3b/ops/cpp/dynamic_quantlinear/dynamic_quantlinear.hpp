...
private:
  ...
  void dynamicquantlinear<InT, OutT>::input_quant(int8_t *output,
                                                  ...
                                                  float scale);
  void dynamicquantlinear<InT, OutT>::output_dequant(
      OutT *output, int32_t *input, std::tuple<int, int> &shape, float scale);
  void dynamicquantlinear<InT, OutT>::log_profile_summary();
...
public:
  ...
  ~dynamicquantlinear();
  void
  dynamicquantlinear::initialize_weights_bin(std::string bin_file,
                                             std::tuple<int, int> &wt_shape);
  void dynamicquantlinear::initialize_weights_data(
      InT *wts, std::tuple<int, int> &wt_shape, float scale);
  ...
  for (int i = 0; i < std::get<0>(wt_shape); i++) {
    for (int j = 0; j < std::get<1>(wt_shape); j++)
      data[i * std::get<1>(wt_shape) + j] =
          std::round(wts[i * std::get<1>(wt_shape) + j] / scale);
  }

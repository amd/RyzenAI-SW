...
OPT_MASK_TILE_MIN_VAL = -60 # To prevent exp_() from lagging due to big absolute input values. Might vary with different models and precisions (currently works for all test cases).
...
class OPTFlashAttention(OPTAttention):
    ...
    def _init_tensors(self, bsz, tgt_len, lh, lq, lk, dtype):
        ...
        self.tile_min = torch.ones(self.mask.shape) * OPT_MASK_TILE_MIN_VAL
    ...
    def forward(
        ...
        for h in range(self.num_heads // lh):
            ...
            for i in range(tgt_len // lq):
                self.max[0].fill_(-1e20)
                self.max[1].fill_(-1e20)
                self.sum[0].zero_()
                self.sum[1].zero_()
                self.qkv.zero_()
                ii = (i*lq)

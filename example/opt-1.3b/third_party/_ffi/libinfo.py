...
def find_lib_path(name=None, search_path=None, optional=False):
    ...
    if name is not None:
        ...
        if sys.platform.startswith("win32"):
            lib_dll_path = [os.path.join(p, "libtvm_ipu.dll") for p in dll_path] + [
                os.path.join(p, "tvm_ipu.dll") for p in dll_path
            ]
            runtime_dll_path = [os.path.join(p, "libtvm_ipu_runtime.dll") for p in dll_path] + [
                os.path.join(p, "tvm_ipu_runtime.dll") for p in dll_path
            ]
        elif sys.platform.startswith("darwin"):
            lib_dll_path = [os.path.join(p, "libtvm_ipu.dylib") for p in dll_path]
            runtime_dll_path = [os.path.join(p, "libtvm_ipu_runtime.dylib") for p in dll_path]
        else:
            lib_dll_path = [os.path.join(p, "libtvm_ipu.so") for p in dll_path]
            runtime_dll_path = [os.path.join(p, "libtvm_ipu_runtime.so") for p in dll_path]
...
__version__ = "0.8.dev0+aie.0.2.4"

 ...
 */
class CVML_SDK_EXPORT_CORE Image {
 public:
  /**
  ...
  /**
   * Move constructor.
   * Constructs the image by transferring ownership of the resources from the source
   * image (other), leaving the source in a valid but unspecified state.
   *
   * @param other Source image from which to move
   */
  Image(Image&& other) noexcept;

  /**
   * Move assignment operator.
   * Transfer ownership of the resources from the source image (other) to the
   * destination image, leaving the source in a valid but unspecified state.
   *
   * @param other Source image from which to move
   * @return Reference to the destination image (this)
   */

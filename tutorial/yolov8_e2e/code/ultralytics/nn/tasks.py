...
from ultralytics.yolo.utils.checks import check_requirements, check_yaml, check_imgsz
...
class BaseModel(nn.Module):
    ...
    def forward(self, x, augment=False, profile=False, visualize=False, quant=False):
        ...
        """
        return self._forward_once(x, augment, profile, visualize, quant=False)

    def _forward_once(self, x, augment=False, profile=False, visualize=False, quant=False):
        """
...
class DetectionModel(BaseModel):
    ...
    def __init__(self, cfg='./ultralytics/models/v8/yolov8m.yaml', ch=3, nc=None, verbose=True):  # model, input channels, number of classes
        super().__init__()
        ...
        self.quantstub = QuantStub()
        self.dequantstub = DeQuantStub()
    ...
    def forward(self, x, augment=False, profile=False, visualize=False, quant=False):
        ...
        x = self._forward_once(x, augment, profile, visualize, quant)  # single-scale inference, train
        if quant:
...
def attempt_load(weight, device=None, inplace=True, fuse=False, imgsz=640, force_reexport_deployable_model=False):
    # Loads a single model weight
    ckpt = torch_safe_load(weight)  # load ckpt
    args = {**DEFAULT_CFG_DICT, **ckpt['train_args']}  # combine model and default args, preferring model args
    model_ckpt = (ckpt.get('ema') or ckpt['model']).to(device).float()  # FP32 model
    if args['model'].endswith(".yaml"):
        cfg = args['model']
    else:
        cfg = args['model'].split('/')[-1].replace('pt', 'yaml')
    model = DetectionModel(cfg=cfg, ch=3, nc=80, verbose=True)
    ...
    if 'qat_model_quant_info' in ckpt:
        if force_reexport_deployable_model:
            ...
            imgsz = args['imgsz']
            imgsz = check_imgsz(imgsz, gs)  # verify imgsz is gs-multiple
            im = torch.zeros(1, 3, imgsz, imgsz).to(next(model_ckpt.parameters()).device)  # image size(1,3,320,192) BCHW iDetection
            # dry run
            for _ in range(2):
                y = model(im)  # dry runs
            ...
            calib_dir = Path(weight).parent / 'nndct_quant'

...
class AutoBackend(nn.Module):
    ...
    def __init__(self, weights='yolov8n.pt', device=torch.device('cpu'), dnn=False, data=None, fp16=False, fuse=True):
        ...
        if nn_module:
            ...
            from ultralytics.nn.tasks import attempt_load, attempt_load_weights
            model, ckpt = attempt_load(weights if isinstance(weights, list) else w,
                                         device=device,
                                         inplace=True,
                                         fuse=fuse)
            stride = max(int(model.stride.max()), 32)  # model stride
    ...
    def forward(self, im, augment=False, visualize=False, quant=False):
        """
        ...
        if self.pt or self.nn_module:  # PyTorch
            y = self.model(im, augment=augment, visualize=visualize, quant=quant) if augment or visualize else self.model(im, quant=quant)

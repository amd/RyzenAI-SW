...
#include <math.h>
#include <float.h>
#include "bfp/cuda/nndct_fix_kernels.cuh"
#include "bfp/cuda/nndct_fix_kernels.h"
#include "bfp/cuda/nndct_cuda_math.h"
#include "bfp/cuda/nndct_cu_utils.h"
#include "bfp/cuda/table_data.h"
...
__global__ static void _cuda_vai_round(const int N, 
  ...
  NNDCT_KERNEL_LOOP(index, N){
    int result_ = 0;
    _vai_round_device(src[index], result_, method, index);
    dst[index] = result_;
  }
}
...
template<typename Dtype>
__global__ static void _fix_neuron_v2_2d(const int N_row, 
                                         const int N_col,
                                         const Dtype* src, 
                                         Dtype* dst, 
                                         int val_min,
                                         ...
                                         int keep_scale, 
                                         int method){
  NNDCT_KERNEL_LOOP(index, N_row*N_col){
    //method: 
    //1: dummy
    //2: for CNN feature map
    //3: for weights and bias
    //4: for RNN feature map
    int result_ = 0;
    int s_index = index/N_col;
    //printf( "$$$$$$$$$$$ index: %d N_row: %d N_col: %d s_index: %d\n", index, N_row, N_col, s_index); 
    Dtype val_amp = 1.0/scale[s_index];
    _fix_neuron_v2_device(src[index], 
                          result_, 
                          val_min,
                          val_max, 
    ...
    //printf( "$$$$$$$$$$$ result: %d zero_point: %d keep_scale: %d\n", result_, zero_point, keep_scale); 
    if(0 != keep_scale)
      dst[index] = (Dtype(result_)-Dtype(zero_point[s_index])) * (1 / val_amp);
    else
      dst[index] = result_;
  }
}
...
template<typename Dtype>
__global__ static void _fix_neuron_v2_2d_v2(const int N_row, 
                                            const int N_col,
                                            const Dtype* src, 
                                            Dtype* dst, 
                                            int val_min,
                                            ...
                                            int keep_scale, 
                                            int method){
  NNDCT_KERNEL_LOOP_2D(i, j, N_row, N_col){
  //NNDCT_KERNEL_LOOP(index, N_row*N_col){
    //method: 
    //1: dummy
    //2: for CNN feature map
    ...
    int result_ = 0;
    //printf( "$$$$$$$$$$$ index: %d N_row: %d N_col: %d s_index: %d\n", index, N_row, N_col, s_index); 
    Dtype val_amp = 1.0/scale[i];
    _fix_neuron_v2_device(src[i*N_col+j], 
                          result_, 
                          val_min,
                          val_max, 
                          val_amp, 
    ...
    //printf( "$$$$$$$$$$$ result: %d zero_point: %d keep_scale: %d\n", result_, zero_point, keep_scale); 
    if(0 != keep_scale)
      dst[i*N_col+j] = (Dtype(result_)-Dtype(zero_point[i])) * (1 / val_amp);
    else
      dst[i*N_col+j] = result_;
  }
}
...
void cuda_fix_neuron_v2_2d(const int N_row, 
                           ...
                           int keep_scale, 
                           int method){
  //dim3 NNDCT_CUDA_NUM_THREADS_2D(NNDCT_CUDA_NUM_THREADS_ROW, NNDCT_CUDA_NUM_THREADS_COL);
  //_fix_neuron_v2_2d_v2<<<NNDCT_GET_BLOCKS_2D(N_row, N_col),NNDCT_CUDA_NUM_THREADS_2D>>>(
  _fix_neuron_v2_2d<<<NNDCT_GET_BLOCKS(N_row*N_col),NNDCT_CUDA_NUM_THREADS>>>(
    N_row,
    N_col, 
    src, 
    dst, 
...
template
void cuda_fix_neuron_v2_2d<float>(const int N_row, 
                                  const int N_col, 
                                  const float* src,
                                  float* dst, 
                                  int val_min,
                                  ...
                                  int method);
template
void cuda_fix_neuron_v2_2d<double>(const int N_row, 
                                   const int N_col, 
                                   const double* src,
                                   double* dst, 
                                   int val_min,

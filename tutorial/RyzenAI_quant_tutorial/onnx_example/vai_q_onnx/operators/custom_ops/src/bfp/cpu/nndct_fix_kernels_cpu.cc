...
static void _cpu_vai_round(const int N, 
  ...
  for(int index=0; index < N; index ++){
    int result_ = 0;
    _vai_round_cpu(src[index], result_, method, index);
    dst[index] = result_;
  }
}
...
template<typename Dtype>
static void _fix_neuron_v2_2d(const int N_row,
                              const int N_col,
                              const Dtype* src, 
                              Dtype* dst, 
                              int val_min,
  ...
  for(int i=0; i < N_row*N_col; i ++){
    ...
    int result_ = 0;
    int j = i / N_col;
    Dtype val_amp = 1.0/scale[j];
    _fix_neuron_v2_cpu(src[i], 
                        result_, 
                        val_min,
                        val_max, 
                        val_amp, 
                        ...
                        i);
    if(0 != keep_scale)
      dst[i] = (Dtype(result_)-Dtype(zero_point[j])) * (1 / val_amp);
    else
      dst[i] = result_;
  }
}
...
template
void cpu_fix_neuron_v2_2d<float>(const int N_row, 
                                 const int N_col, 
                                 const float* src,
                                 float* dst, 
                                 int val_min,
                                 ...
                                 int method);
template
void cpu_fix_neuron_v2_2d<double>(const int N_row, 
                                  const int N_col, 
                                  const double* src,
                                  double* dst, 
                                  int val_min,

...
#ifdef __HIP_PLATFORM_AMD__
__device__ inline int _stochastic_round(const float& val, int seed){
    hiprandState_t state;
    hiprand_init(static_cast<unsigned long long>(clock()) + seed, 0, 0, &state);
    return floor(val + hiprand_uniform(&state));
}

__device__ inline int _stochastic_round(const double& val, int seed){
    hiprandState_t state;
    hiprand_init(static_cast<unsigned long long>(clock()) + seed, 0, 0, &state);
    return floor(val + hiprand_uniform_double(&state));
}
#else
__device__ inline int _stochastic_round(const float& val, int seed){
    curandState state;
    curand_init(static_cast<unsigned long long>(clock()) + seed, 0, 0, &state);
    return floor(val + curand_uniform(&state));
}

__device__ inline int _stochastic_round(const double& val, int seed){
    curandState state;
    curand_init(static_cast<unsigned long long>(clock()) + seed, 0, 0, &state);
    return floor(val + curand_uniform_double(&state));
}
#endif

template<typename Real>
__device__ void _vai_round_device(const Real& x, int& y, int method, int seed){
  if(2==method){  //half_up
...
__device__ void _fix_neuron_v2_device(const Real& src,int& res,
  ...
  // 8: half_even 
  // 9: stochastic_round
  _vai_round_device(res_real_, res, method, seed);

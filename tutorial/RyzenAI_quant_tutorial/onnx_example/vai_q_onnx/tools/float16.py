...
def _npint_to_float(np_list):
    ...
    '''
    return [
        _.astype(np.uint16).view(np.float16).astype(np.float32).item()
        for _ in np_list
    ]
...
def convert_np_to_float16(np_array, min_positive_val=1e-7, max_finite_val=1e4):
    ...
    np_array = np.where(between(0, np_array, min_positive_val),
                        min_positive_val, np_array)
    np_array = np.where(between(-min_positive_val, np_array, 0),
                        -min_positive_val, np_array)
    np_array = np.where(between(max_finite_val, np_array, float('inf')),
                        max_finite_val, np_array)
    np_array = np.where(between(float('-inf'), np_array, -max_finite_val),
                        -max_finite_val, np_array)
...
def convert_tensor_float_to_float16(tensor,
    ...
    if not isinstance(tensor, onnx_proto.TensorProto):
        raise ValueError(
            'Expected input type is an ONNX TensorProto but got %s' %
            type(tensor))
    ...
    if tensor.data_type == onnx_proto.TensorProto.FLOAT:
        ...
        if tensor.raw_data:
            ...
            float16_list = convert_np_to_float16(float32_list, min_positive_val,
                                                 max_finite_val)
...
DEFAULT_OP_BLOCK_LIST_FP16 = [
    ...
    'NonMaxSuppression', 'TopK', 'RoiAlign', 'Resize', 'Range', 'CumSum', 'Min',
    'Max', 'Upsample'
]

DEFAULT_OP_BLOCK_LIST_FP32 = []
...
def convert_float_to_float16(model,
    ...
    if not disable_shape_infer and pv.Version(
            onnx.__version__) >= pv.Version('1.2'):
        try:
    ...
    if not isinstance(model, onnx_proto.ModelProto):
        raise ValueError(
            'Expected model type is an ONNX ModelProto but got %s' %
            type(model))

    # create blocklists
    if op_block_list is None:
        op_block_list = DEFAULT_OP_BLOCK_LIST_FP16
    ...
    if keep_io_types:
        for i, n in enumerate(model.graph.input):
            if n.type.tensor_type.elem_type == onnx_proto.TensorProto.FLOAT:
                ...
                new_node = [
                    helper.make_node('Cast', [n.name], [output_name],
                                     to=10,
                                     name=node_name)
                ]
        ...
        for i, n in enumerate(model.graph.output):
            if n.type.tensor_type.elem_type == onnx_proto.TensorProto.FLOAT:
                ...
                new_value_info.type.tensor_type.elem_type = onnx_proto.TensorProto.FLOAT16
                new_node = [
                    helper.make_node('Cast', [input_name], [n.name],
                                     to=1,
                                     name=node_name)
                ]
                model.graph.node.extend(new_node)
        ...
        for q in queue:
            ...
            if isinstance(q, onnx_proto.AttributeProto):
                ...
                for n in q.graphs:
                    next_level.append(n)
                q.t.CopyFrom(
                    convert_tensor_float_to_float16(q.t, min_positive_val,
                                                    max_finite_val))
                for n in q.tensors:
                    n = convert_tensor_float_to_float16(n, min_positive_val,
                                                        max_finite_val)
        ...
        for i in range(len(node.input)):
            ...
            for value_info in value_info_list:
                if input == value_info.name:
                    ...
                    graph = node_dict[
                        node.
                        name]  # get the correct graph instead of the global graph
                    ...
                    new_node = [
                        helper.make_node('Cast', [input], [output_name],
                                         to=1,
                                         name=node_name)
                    ]
        ...
        for i in range(len(node.output)):
            ...
            for value_info in value_info_list:
                if output == value_info.name:
                    ...
                    new_node = [
                        helper.make_node('Cast', [input_name], [output],
                                         to=10,
                                         name=node_name)
                    ]
                    graph.node.extend(new_node)
...
def convert_float_to_float16_model_path(model_path,
                                        min_positive_val=1e-7,
                                        max_finite_val=1e4,
                                        keep_io_types=False):
    '''
    ...
    *But this function can be applied to all model sizes.
    ...
    if pv.Version(onnx.__version__) >= pv.Version('1.8'):
        try:
            ...
            with tempfile.NamedTemporaryFile(
                    dir=os.path.dirname(model_path)) as tmpfile:
    ...
    return convert_float_to_float16(model, min_positive_val, max_finite_val,
    ...
    def find_first_node(output2node_dict):
        for node in org_nodes:
            is_not_first_node = any(
                item in output2node_dict for item in node.input)
...
def sort_topology(graph_proto):
    ...
    for node in graph_proto.node:
        for attr in node.attribute:
            if isinstance(attr.g,
                          onnx_proto.GraphProto) and len(attr.g.node) > 0:
...
def convert_np_to_float(np_array, min_positive_val=1e-7, max_finite_val=1e4):
    '''
    Convert float16 numpy array to float32 without changing sign or finiteness.
    Similar for negative values. NaN, 0, inf, and -inf are unchanged.
    '''
    return np.float32(np_array)


def convert_tensor_float16_to_float(tensor):
    '''
    ...
    if tensor.data_type == onnx_proto.TensorProto.FLOAT16:
        ...
        if tensor.raw_data:
            # convert n.raw_data to float
            float16_list = np.fromstring(tensor.raw_data, dtype='float16')
            # convert float to float16
            float32_list = convert_np_to_float(float16_list)
            # convert float16 to bytes and write back to raw_data
            tensor.raw_data = float32_list.tostring()
    return tensor


def convert_float16_to_float(model,
                             disable_shape_infer=False,
                             op_block_list=None,
                             node_block_list=None):
    '''
